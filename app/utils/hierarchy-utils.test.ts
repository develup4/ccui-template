...
export function runHierarchyTests(): void {
  ...
  tests.forEach((test, index) => {
    ...
    const result = findInstanceWithContext(sampleIPInstanceHierarchy, test.path);
    ...
    if (targetMatch && parentMatch && childrenMatch) {
      ...
      console.log('❌ FAIL');
      console.log(`  Expected: target="${test.expectedTarget}", parent="${test.expectedParent}", children=${test.expectedChildrenCount}`);
      console.log(`  Got:      target="${targetName}", parent="${parentName}", children=${childrenCount}`);
    }
    console.log('---\n');
  });
