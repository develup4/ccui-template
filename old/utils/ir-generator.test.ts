...
describe("IR Generator", () => {
  ...
  it("should handle custom type correctly", () => {
    ...
    const memorySubsystem = ir.components?.find(
      (c) => c.name === "Memory Subsystem"
    );
    const ddr4Controller = memorySubsystem?.components?.find(
      (c) => c.name === "DDR4 Controller"
    );
  ...
  it("should use default property values when instance has no override", () => {
    ...
    const cpuCluster = ir.components?.find((c) => c.name === "CPU Cluster");
    const core0 = cpuCluster?.components?.find(
      (c) => c.name === "ARM Cortex-A78"
    );
